...
// 2) Construcción de la consulta base — seleccionamos la columna de texto `estado`
$sql = "
    SELECT
        i.id,
        i.tipo_incidencia,
...
// 3) Montar cláusulas dinámicas solo para docente
$clauses = ["i.usuario_id = ?"];
$params  = [$userId];
$types   = "i";

// 4) Si hay filtro de estado, lo añadimos
if ($estadoFiltro !== null) {
    $clauses[]  = "UPPER(i.estado) = ?";
    $types     .= "s";
    $params[]   = $estadoFiltro;
}

// 5) Si hay cláusulas, unirlas en WHERE
if (!empty($clauses)) {
    ...
    <div class="dashboard-container">
        <div class="dashboard-header">
            <h1 class="dashboard-title">
                <i class="fas fa-tasks"></i> Gestión de Incidencias
            </h1>
        ...
        <div class="filters-container">
            <form method="get" action="consulta.php">
                ...
                >
                    <option value="" <?= $estadoFiltro === null ? 'selected' : '' ?>>
                        Todas las incidencias
                    </option>
                    <option value="PENDIENTE" <?= $estadoFiltro === 'PENDIENTE' ? 'selected' : '' ?>>
                        Pendientes
                    </option>
                    <option value="ACEPTADO" <?= $estadoFiltro === 'ACEPTADO' ? 'selected' : '' ?>>
                        Aceptadas
                    </option>
                    <option value="RECHAZADO" <?= $estadoFiltro === 'RECHAZADO' ? 'selected' : '' ?>>
                        Rechazadas
                    </option>
                    <option value="MODIFICAR" <?= $estadoFiltro === 'MODIFICAR' ? 'selected' : '' ?>>
                        A modificar
                    </option>
                </select>
            </form>
        ...
        <div class="incidencias-container">
            <?php if ($result->num_rows === 0): ?>
                <div class="no-incidencias">
                    ...
                    <p><?= $estadoFiltro!==null ? "No tienes incidencias en estado '{$estadoFiltro}'." : "No tienes incidencias registradas." ?></p>
                </div>
            <?php else: ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <div class="incidencia-card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="incidencia-meta">
                                ...
                                <div class="estado-container">
                                    <span>Estado:</span>
                                    <div class="estado-badge estado-<?= $row['estado'] ?>">
                        ...
                        <div class="card-footer">
                            <a href="ver_pdf.php?id=<?= $row['id'] ?>" target="_blank" class="btn-action btn-review">
                                <i class="fas fa-eye"></i> Revisar
                            </a>

                            <?php if (strtoupper($row['estado']) === 'MODIFICAR'): ?>
